...
def compile_gene_structure(filename, gene_search):
    ...
    with open(filename, 'r') as f:
        unsorted_gene_structure = [] #Build the initial list which will store gene segments
        ...
        for line in f:
            if line[0] == 'c': #Only reads appropriate lines containing gene information
                line_info = line.split("\t") #Spits the values seperated by tabs

                if line_info[2] == "gene": 
                    if gene_found == True: #If this condition is met, the intended gene has been found previously, and further search is unnecessary

                        segment_count = 0
                        loop_count = 0
                        if strand == "+": #If Forward strand, sorting will be different 

                            #Defines new list containing the segments compiled below, but sorted in ascending co-ordinate order
                            gene_structure = sorted(unsorted_gene_structure, key=lambda k: k['Beginning']) 
                            ...
                            for elem in gene_structure:
                                if segment_count ==0:
                                    indexes = {"Beginning_Index" : 0, "End_Index" : (elem["Length"] - 1)}
                                    elem.update(indexes)

                                elif segment_count != 0:
                                    start_ind = ((gene_structure[segment_count - 1])["End_Index"] + 1) 
                                    indexes = {"Beginning_Index" : start_ind, "End_Index" : (start_ind + elem["Length"] - 1)}
                                    elem.update(indexes)
                            ...
                            gene_structure = sorted(unsorted_gene_structure, key=lambda k: k['Beginning'], reverse=True) 
                ...
                if (gene_search == gene_name) or (gene_search == gene_id) or (gene_search == transcript_name):
                    ...
                    if (line_info[2] == "CDS") or (line_info[2] == "five_prime_UTR") or (line_info[2] == "three_prime_UTR"):
                        if strand == "+": #Fw/Rv strand determines assignment of beginning and end
...
def reading_frame(seq, init_index):
    ...
    return[(seq[index:index + 3]) for index in range(init_index, len(seq) - 2, 3)] #Return series of base triplets
...
def uORF_gene_region(gene_structure, index):
    ...
    for segment in gene_structure:
        if (index >= segment["Beginning_Index"]) and (index <= segment["End_Index"]):
            ...
            region = region_name + " :: " + str(segment["Beginning"]) + " - " + str(segment["End"])
            return(region)
...
# - Calculate Kozak
    ...
    for codon in read_frames[n]:
        ...
        if codon == "ATG":
            ...
            start_index = (n + (codon_count * 3)) #Gets sequence index of the beginning of the start codon
            ...
            print("START", start_index, start_region, (sequence[start_index - 3], sequence[start_index + 3]), kozak_strength)
            ...
            print(start)
            ORF.append(start)
            if (initial_start_found == False): #Captures index of ORF's first start, for later sorting
                orf_start = start_index
                orf_region = start_region
                initial_start_found = True

        if (open_RF == True) and ((codon == "TAA") or (codon == "TAG") or (codon == "TGA")):
            ...
            open_RF = False #Resets the value to indicate that an ORF hasn't been started
            stop = {
                "type" : "stop",
                "index": stop_index,
                "region": stop_region
